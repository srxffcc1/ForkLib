...
import com.yarolegovich.slidingrootnav.transform.menu.CompositeMenuTransformation;
import com.yarolegovich.slidingrootnav.transform.menu.MenuTransformation;
import com.yarolegovich.slidingrootnav.transform.menu.ScaleMenuTransformation;
import com.yarolegovich.slidingrootnav.transform.menu.XTranslationMenuTransformation;
import com.yarolegovich.slidingrootnav.transform.root.CompositeRootTransformation;
import com.yarolegovich.slidingrootnav.transform.root.ElevationRootTransformation;
import com.yarolegovich.slidingrootnav.transform.root.RootTransformation;
import com.yarolegovich.slidingrootnav.transform.root.ScaleRootTransformation;
import com.yarolegovich.slidingrootnav.transform.root.YTranslationRootTransformation;
...
public class SlidingRootNavBuilder {
    ...
    private static final float DEFAULT_END_SCALE = 0.85f;//最终缩放
    private static final int DEFAULT_END_ELEVATION_DP = 8;
    private static final float DEFAULT_DRAG_DIST_DP =0.7f;
    ...
    private List<RootTransformation> roottransformations;

    private List<MenuTransformation> menutransformations;
    ...
    public SlidingRootNavBuilder(Activity activity) {
        ...
        this.roottransformations = new ArrayList<>();
        ...
        this.dragDistance = (int)(mScreenWidth*DEFAULT_DRAG_DIST_DP);
    }

    public SlidingRootNavBuilder setBackgroundRid(int backgroundRid) {
    ...
    public SlidingRootNavBuilder withRootViewScale(@FloatRange(from = 0.01f) float scale) {
        roottransformations.add(new ScaleRootTransformation(scale));
        return this;
    }
    ...
    public SlidingRootNavBuilder withRootViewElevationPx(@IntRange(from = 0) int elevation) {
        roottransformations.add(new ElevationRootTransformation(elevation));
        return this;
    }
    ...
    public SlidingRootNavBuilder withRootViewYTranslationPx(int translation) {
        roottransformations.add(new YTranslationRootTransformation(translation));
        return this;
    }

    public SlidingRootNavBuilder addRootTransformation(RootTransformation transformation) {
        roottransformations.add(transformation);
        return this;
    }
    ...
    public SlidingRootNav inject() {
        ...
        newRoot.setBackgroundResource(backgroundRid);
    ...
    private SlidingRootNavLayout createAndInitNewRoot(View oldRoot) {
        ...
        newRoot.setRootTransformation(createCompositeRootTransformation());
    ...
    private RootTransformation createCompositeRootTransformation() {
        if (roottransformations.isEmpty()) {
            return new CompositeRootTransformation(Arrays.asList(
                    new ScaleRootTransformation(DEFAULT_END_SCALE),
                    new ElevationRootTransformation(dpToPx(DEFAULT_END_ELEVATION_DP))));
        } else {
            return new CompositeRootTransformation(roottransformations);
        }
    }
    private MenuTransformation createCompositeMenuTransformation() {
        if (menutransformations.isEmpty()) {
            return new CompositeMenuTransformation(Arrays.asList(
                    new ScaleMenuTransformation(1),
                    new XTranslationMenuTransformation(0)));
        } else {
            return new CompositeMenuTransformation(menutransformations);
        }
    }
