...
public class MuPDFPageView extends PageView implements MuPDFView {
		...
		if (hit) {
			mPassClick = new AsyncTask<Void,Void,PassClickResult>() {
				...
				protected PassClickResult doInBackground(Void... arg0) {
					return passClickEvent(mPageNumber, docRelX, docRelY);
				}
	...
	public synchronized PassClickResult passClickEvent(int page, float x, float y) {
		boolean changed = mCore.passClickEvent(page, x, y) != 0;

		switch (WidgetType.values()[mCore.getFocusedWidgetType()])
		{
//			case TEXT:
//				return new PassClickResultText(changed, mCore.getFocusedWidgetText());
//			case LISTBOX:
//			case COMBOBOX:
//				return new PassClickResultChoice(changed, mCore.getFocusedWidgetChoiceOptions(), mCore.getFocusedWidgetChoiceSelected());
//			case SIGNATURE:
//				return new PassClickResultSignature(changed, mCore.getFocusedWidgetSignatureState());
			default:
