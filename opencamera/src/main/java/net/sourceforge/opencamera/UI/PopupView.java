...
public class PopupView extends LinearLayout {
	...
	public PopupView(Context context) {
		...
		final OpMainActivity main_activity = (OpMainActivity)this.getContext();
		...
		if( preview.isVideo() && preview.isTakingPhoto() ) {
			...
			final List<OpApplicationInterface.PhotoMode> photo_mode_values = new ArrayList<>();
			photo_modes.add( getResources().getString(R.string.photo_mode_standard) );
			photo_mode_values.add( OpApplicationInterface.PhotoMode.Standard );
			if( main_activity.supportsDRO() ) {
				photo_modes.add( getResources().getString(R.string.photo_mode_dro) );
				photo_mode_values.add( OpApplicationInterface.PhotoMode.DRO );
			}
    		if( main_activity.supportsHDR() ) {
    			photo_modes.add( getResources().getString(R.string.photo_mode_hdr) );
    			photo_mode_values.add( OpApplicationInterface.PhotoMode.HDR );
    		}
    		if( main_activity.supportsExpoBracketing() ) {
    			photo_modes.add( getResources().getString(R.string.photo_mode_expo_bracketing) );
    			photo_mode_values.add( OpApplicationInterface.PhotoMode.ExpoBracketing );
    		}
    		if( photo_modes.size() > 1 ) {
    			OpApplicationInterface.PhotoMode photo_mode = main_activity.getApplicationInterface().getPhotoMode();
            	...
            	addButtonOptionsToPopup(photo_modes, -1, -1, "", current_mode, "TEST_PHOTO_MODE", new ButtonOptionsPopupListener() {
        			...
        			public void onClick(String option) {
        				...
        				if( option_id == -1 ) {
    						...
    						OpApplicationInterface.PhotoMode new_photo_mode = photo_mode_values.get(option_id);
    						String toast_message = option;
    						if( new_photo_mode == OpApplicationInterface.PhotoMode.ExpoBracketing )
    							toast_message = getResources().getString(R.string.photo_mode_expo_bracketing_full);
    	    				final SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(main_activity);
    						SharedPreferences.Editor editor = sharedPreferences.edit();
    						if( new_photo_mode == OpApplicationInterface.PhotoMode.Standard ) {
        						editor.putString(PreferenceKeys.getPhotoModePreferenceKey(), "preference_photo_mode_std");
    						}
							else if( new_photo_mode == OpApplicationInterface.PhotoMode.DRO ) {
								editor.putString(PreferenceKeys.getPhotoModePreferenceKey(), "preference_photo_mode_dro");
							}
							else if( new_photo_mode == OpApplicationInterface.PhotoMode.HDR ) {
								editor.putString(PreferenceKeys.getPhotoModePreferenceKey(), "preference_photo_mode_hdr");
							}
    						else if( new_photo_mode == OpApplicationInterface.PhotoMode.ExpoBracketing ) {
    						...
    						if( new_photo_mode == OpApplicationInterface.PhotoMode.HDR ) {
