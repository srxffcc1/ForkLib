...
public class VerticalStepView extends LinearLayout implements VerticalStepViewIndicator.OnDrawIndicatorListener
{
    ...
    private VerticalStepViewIndicator.OnItemClickListener onItemClickListener;
    ...
    public void setOnItemClickListener(VerticalStepViewIndicator.OnItemClickListener onItemClickListener) {
            ...
            if(mTexts != null && complectedXPosition != null && complectedXPosition.size() > 0)
            {
                for(int i = 0; i < mTexts.size(); i++)
                {
                    ...
                    if(i <= mComplectingPosition)
                    {
                        ...
                        mTextView = (TextView) LayoutInflater.from(getContext()).inflate(R.layout.completetext,null);
                    } else
                    {
                        mTextView = (TextView) LayoutInflater.from(getContext()).inflate(R.layout.nocompletetext,null);
                    }
                    mTextView.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            if(onItemClickListener!=null){
                                onItemClickListener.onItemClick(v,tmpi);
                            }
                        }
                    });
                    ...
                    mTextView.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.MATCH_PARENT));
